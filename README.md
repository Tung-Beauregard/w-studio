...
新增專案時，請依需要在 `assets/app.js` 加入示意圖及介紹。已提供使用入口的專案沿用 `demos` 三步驟動畫；研發中的 Mycelint 以 `site-data.js` 的 `details` 提供站內詳情。現有五個專案均具備介紹，全部專案數量由資料自動更新；分類按鈕在 `index.html` 設定。沒有有效網址的專案不會顯示外部入口，未提供原始碼網址的專案不會顯示原始碼按鈕。
